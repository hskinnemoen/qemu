...
.. blockdiag::
    ...
    blockdiag main_processor_ipmi {
        ...
        group {
            ...
            TYPE_IPMI_INTERFACE <-> TYPE_IPMI_BMC;

            group {
                orientation = portrait

                TYPE_IPMI_INTERFACE [class = "msgif"];
                isa-ipmi-kcs [class="msgif", stacked];

                TYPE_IPMI_INTERFACE <- isa-ipmi-kcs [hstyle = generalization];
            }
            ...
            group {
                ...
                TYPE_IPMI_BMC [class = "bmc"];
                ipmi-bmc-sim [class="bmc"];
                ipmi-bmc-extern [class="bmc"];

                TYPE_IPMI_BMC <- ipmi-bmc-sim [hstyle = generalization];
                TYPE_IPMI_BMC <- ipmi-bmc-extern [hstyle = generalization];
            }
...
or i2c bus, and in QEMU, they all need to implement ``TYPE_IPMI_INTERFACE``.
...
device, that is a subclass of ``TYPE_IPMI_BMC``. This type of device is called
